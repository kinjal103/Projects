...
$result = mysql_query("SELECT * FROM `wholesaler"."$team_no"."`");
$cur_week_arr = mysql_fetch_array(mysql_query("SELECT `week` FROM `current_week`"));
$cur_week = $cur_week_arr['week'];
$inp_ord_res = mysql_query("SELECT `order_out` FROM `retailer"."$team_no"."` WHERE `order_out_week`='$cur_week'");
...
$inp_del_res = mysql_query("SELECT `del_out` FROM `distributor"."$team_no"."` WHERE `del_out_week`='$cur_week'");
...
else
{
	...
	if($check['wholesaler']==0)
	{
		mysql_query("UPDATE `teams` SET `wholesaler`='1' WHERE `team_no`='$team_no'");
		$check = mysql_fetch_array(mysql_query("SELECT * FROM `teams` WHERE `team_no`='$team_no'"));
		if(($check['retailer']==1)&&($check['distributor']==1)&&($check['factory']==1))
		{
...
<body>
<br/>
<form action="wholesaler_backend.php" method="post"><center>
<table>
<tr><td>Order to distributor:</td>
<td><input type="number" size="10" name="order_out"/></td></tr>
<tr><td>Delivery to retailer:</td>
